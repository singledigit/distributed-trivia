  ...
  <div class="cats" v-if="authenticated">
    ...
    <div class="cats-inner">
      ...
      <header class="header">
        ...
        <nav class="nav">
          <RouterLink to="/controller" class="nav-link">Game</RouterLink>
          <RouterLink to="/categories" class="nav-link nav-active">Categories</RouterLink>
        </nav>
...
.btn-signout {
  background: transparent; border: 1px solid var(--border-subtle); border-radius: var(--radius-full);
  color: var(--text-muted); font-family: var(--font-display); font-size: 12px; padding: 4px 12px;
  cursor: pointer; transition: all 0.2s;
}
.btn-signout:hover { border-color: var(--border-medium); color: var(--text-secondary); }
